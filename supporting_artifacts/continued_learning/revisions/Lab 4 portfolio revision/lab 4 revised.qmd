...
### Portfolio Revision: Question 6

## 6) In which regions is the price of organic versus conventional avocados most different? \[Not revised\] {style="color: darkolivegreen"}
...
*In this class we don't use the `match()` function. I would like for you to accomplish this task using `pivot_wider()`. Give this another go!*\

## 6) In which regions is the price of organic versus conventional avocados most different? \[Revised\] {style="color: darkolivegreen"}
...
avocados_california_regions |>
  ...
  pivot_wider(names_from = type,
              values_from = avg_price_by_type) |> #added pivot wider to pipe as portfolio                                                        revision
  summarise(
    diff_price = round(abs(
      conventional - organic), digits = 2))
